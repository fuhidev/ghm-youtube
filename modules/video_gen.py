...
def create_video(image_path, audio_path, output_path, subtitle_path=None):
    ...
    if subtitle_path and os.path.exists(subtitle_path):
        ...
        subtitle_path_escaped = str(subtitle_absolute_path).replace('\\', '/').replace(':', '\\:')
        
        # Sử dụng subtitles filter với các tùy chọn để phụ đề lớn, đậm, màu trắng viền xanh, ở giữa
        subtitle_options = f"force_style='Fontname=Arial,Fontsize=48,PrimaryColour=&HFFFFFF,OutlineColour=&H0000FF,BorderStyle=1,Outline=3,Shadow=0,Alignment=2,MarginV=35'"
